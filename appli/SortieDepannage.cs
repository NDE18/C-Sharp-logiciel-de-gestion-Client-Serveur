...
namespace appli
{
    public partial class SortieDepannage : DevComponents.DotNetBar.Office2007Form
    {
        DataTable table;
        MySqlDataAdapter da = new MySqlDataAdapter();

        public SortieDepannage()
        {
            InitializeComponent();
        ...
        public void BEnregSortie_Executed(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                else
                {
                    ...
                    s.SetMotif("Dépannage numéro " + numFabrication.Text);
        ...
        private void chargerNumDepannage()
        {
        ...
        {
            ConnexionDB c = new ConnexionDB();
            string query = "Select date_enregistrer,nom_employe,diagnostic from depannage INNER JOIN employe ON (num_employe = numero_employe) WHERE num_depannage = '" + valeur + "'";
            MySqlCommand cmd = new MySqlCommand(query, c.con);
            MySqlDataReader reader;
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    this.TDescription.Text = reader.GetString("diagnostic");
                }
                reader.Close();
                c.con.Close();
            }
